...
export default function CompanyPop(props){
    ...
    /* useEffect(() => {
        let filteredList    =   (orgCompanies || []).filter(item => (item.name || '').toLowerCase().includes(searchTxt.toLowerCase()));
    ...
    const getCompanies = async (pageno) => {
        try{
            ...
            setLoading(true);
            const payload = await callApi('GET', `/company/list/`, data);
            if(payload.data) {
